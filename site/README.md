...
- 内置 `Orbital Organ` 是浏览器实时生成的原创氛围声景，不含外部采样。
- “载入本地授权音频”只在当前浏览器会话读取用户选择的文件，不上传、不复制进项目。
- 用户提到的《星际穿越》麦田段落曲目已确认是 Hans Zimmer 的 `Cornfield Chase`；本站只提供 WaterTower Music 官方试听链接，不打包受版权保护的电影原声。
